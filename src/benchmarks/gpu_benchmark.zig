...
fn benchmarkGpuVsCpu(allocator: std.mem.Allocator, db_context: *DatabaseContext) !void {
    ...
    const adv_planner = try AdvancedQueryPlanner.init(allocator);
    ...
    for (queries) |query| {
        ...
        // Optimize with standard planner
        const physical_plan = try planner.optimize(standard_planner, logical_plan);
        ...
        const ast_gpu = try adv_planner.parse(query);
        defer ast_gpu.deinit();

        const logical_plan_gpu = try adv_planner.plan(ast_gpu);
        defer logical_plan_gpu.deinit();

        // Force GPU execution
        adv_planner.setForceGpu(true);
        const physical_plan_gpu = try adv_planner.optimize(logical_plan_gpu);
        defer physical_plan_gpu.deinit();
        adv_planner.setForceGpu(false);
    ...
    gpu_vs_cpu_results = .{
        .query_names = try query_names.toOwnedSlice(),
        .cpu_times = try cpu_times.toOwnedSlice(),
        .gpu_times = try gpu_times.toOwnedSlice(),
    };
}
...
fn benchmarkQueryTypes(allocator: std.mem.Allocator, db_context: *DatabaseContext) !void {
    ...
    // Force GPU execution
    adv_planner.setForceGpu(true);
    defer adv_planner.setForceGpu(false);
    ...
    for (query_types) |query_type| {
        ...
        const ast = try adv_planner.parse(query_type.query);
        defer ast.deinit();

        const logical_plan = try adv_planner.plan(ast);
        defer logical_plan.deinit();
    ...
    // Store results for later writing to file
    query_type_results = .{
        .query_type_names = try query_type_names.toOwnedSlice(),
...
fn benchmarkDataSizes(allocator: std.mem.Allocator, db_context: *DatabaseContext) !void {
    ...
    for (data_sizes) |data_size| {
        ...
        const ast = try adv_planner.parse(query);
        defer ast.deinit();
    ...
    // Store results for later writing to file
    data_size_results = .{
        .data_sizes = try data_size_values.toOwnedSlice(),
        .execution_times = try execution_times.toOwnedSlice(),
    };
}
